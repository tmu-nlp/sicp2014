      ...
      (if proc
          ...
          (if (= (length args) 2)
              (let ((type1 (car type-tags))
                    ...
                    (a1 (car args))
                    (a2 (cadr args)))
                (cond ((find-lower type1 type2 type-tower)
                       (apply-generic op (raise a1) a2))
                      ((find-lower type2 type1 type-tower)
...
(define type-tower (list 'scheme-number 'rational 'rectangular 'complex))

(define (find-lower candidate-type check-type type-tower)
  (define (sub-before item lst)
    (reverse (cdr (member item (reverse lst)))))
    (member candidate-type (sub-before check-type type-tower)))
...
(put 'project '(complex) 
     (lambda (n)
       (make-real (real-part n) 0)))

(put 'project '(rectangular) 
     (lambda (n)
       (let ((real-part (car n)))
                  (make-rational (round real-part) 1))))

(put 'project '(rational)
     (lambda (n)
         (let ((numer (car n))
               (denum (cdr n)))
           (make-scheme-number (round (/ numer denum 1.0))))))
...
(project (make-complex-from-real-imag 8 2))
(project (make-real 8.5 0))
(project (make-rational 8 2))

(define (drop n) 
  (let ((proc (get 'project (list (type-tag n)))))
    (if proc
...
(drop (make-complex-from-real-imag 8 2))
(drop (make-complex-from-real-imag 8 0))
(drop (make-real 8.5 0))
...
(define (add2 x y) (apply-generic2 'add x y))

(newline)
"apply-generic2"
(newline)

(add2 (make-rational 8 2) (make-scheme-number 5)) ;-> dropped!
(add2 (make-rational 8 3) (make-scheme-number 5)) ;-> not dropped, but it is ok!
