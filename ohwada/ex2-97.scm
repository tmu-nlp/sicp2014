...
(define (install-sparse-polynomial-package)
  ...
  ; 多項式の全ての係数の最大公約数を計算する手続き
  (define (coeff-gcd-iter terms)
    (define (iter g l)
      (if (null? l)
          g
          (iter (gcd g (coeff (car l)))
                (cdr l))))
     (iter (coeff (car terms)) (cdr terms)))
  ...
  (define (pseudoremainder-terms a b)
    (let ((i-e (expt (coeff (first-term b))
                     (- (+ 1 (order (first-term a)))
                        (order (first-term b))))))
      (cadr (div-terms
              (mul-term-by-all-terms (make-term 0 i-e) a)
              b))))
...
;-------------------- 追加部分１----------------------------------
  ...
  (define (reduce-poly p1 p2)
    (if (same-variable? (variable p1) (variable p2))
        (let ((r-terms (reduce-terms (term-list p1) (term-list p2))))
          (let ((r-n-terms (car r-terms)) (r-d-terms (cadr r-terms)))
            (if (< (coeff (first-term r-d-terms)) 0)
                (list (make-poly (variable p1) (map reverse-sign r-n-terms))
                      (make-poly (variable p1) (map reverse-sign r-d-terms)))
                (list (make-poly (variable p1) r-n-terms)
                      (make-poly (variable p1) r-d-terms)))))
...
;--------------------- 変更部分-------------------------
  ; make-rat で reduce を使うようにする
  (define (make-rat n d)
    (let ((r-nd (reduce n d)))
      (cons (car r-nd) (cadr r-nd))))
;-------------------------------------------------------
...
; scheme-number が引数の時の reduce を put
(define (tag s) (attach-tag 'scheme-number s))

(put 'reduce '(scheme-number scheme-number)
     (lambda (s1 s2) (map tag (reduce-integers s1 s2))))
...
; 以下 ex2-93 と比較するためのテスト

; (ex2-93 では、(add rf rf) は分母が p1^2 で分子が p1*p2 + p1*p2 のままだったが、
;  こちらでは分子の係数を２倍しただけの当然そうなるべき形になっている)
