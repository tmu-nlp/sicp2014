...
(define (coerce-higher-type items)
  (let ((item1 (car items))
       ...
       (let ((type1 (type-tag item1))
            ...
            (if (eq? type1 type2)
                items
                (let ((tag (higher-type type1 type2))) ;tagにはtype1かtype2どちらかの、塔でいうより高い方のタグが入っている
                     (if (eq? tag type1)
...
(print (higher-type (type-tag i) (type-tag c)))

(print (coerce-higher-type (list ra i)))
(print (coerce-higher-type (list r ra)))
(print (coerce-higher-type (list c ra)))
